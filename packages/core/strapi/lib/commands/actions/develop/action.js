...
module.exports = async ({ build, watchAdmin, polling, browser, disableClient }) => {
...
const primaryProcess = async ({
  ...
  const openClientSite = getOr(true, 'admin.openClientSite')(config);
  ...
  if (!disableClient && openClientSite) {
    try {
      execa('npm', ['run', '-s', 'strapi', 'disable-client'], {
        stdio: 'inherit',
      });
    } catch (err) {
      process.exit(1);
